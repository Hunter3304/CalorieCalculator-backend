...
class BodyCircumferenceServiceTest {
    ...
    void resolvesEachMeasurementFromItsOwnSourceDate() {
        ...
        when(mapper.findLatestForColumn(USER_ID, today, "chest_cm")).thenReturn(chest);
        when(mapper.findLatestForColumn(USER_ID, today, "waist_cm")).thenReturn(waist);

        BodyCircumferenceSnapshotDto snapshot = service.getSnapshot(USER_ID, today);
    ...
    void treatsAllBlankCreateAsNoOp() {
        when(mapper.findByDate(USER_ID, today)).thenReturn(null);

        BodyCircumferenceSnapshotDto snapshot =
                service.save(USER_ID, today, blankRequest(null));

        verify(mapper, never()).insert(anyLong(), any());
        verify(mapper, never()).update(anyLong(), any());
        assertNull(snapshot.recordId());
    }

    @Test
    void insertsOnlyProvidedMeasurementsForOwner() {
        BodyCircumferenceRequest request = new BodyCircumferenceRequest(
                null, new BigDecimal("78.5"), null, null, null, null, null);
        when(mapper.findByDate(USER_ID, today)).thenReturn(null);

        service.save(USER_ID, today, request);

        verify(mapper).insert(eq(USER_ID), argThat(record ->
    ...
    void clearsOneOverrideAndKeepsTheSparseRecord() {
        ...
        when(mapper.findByDate(USER_ID, today)).thenReturn(existing);

        service.save(USER_ID, today, blankRequest(List.of("chest")));

        assertNull(existing.getChestCm());
        assertEquals(new BigDecimal("78.0"), existing.getWaistCm());
        verify(mapper).update(USER_ID, existing);
        verify(mapper, never()).deleteById(USER_ID, 3);
    }
    ...
    void removesRowWhenItsLastOverrideIsCleared() {
        ...
        when(mapper.findByDate(USER_ID, today)).thenReturn(existing);
        when(mapper.deleteById(USER_ID, 4)).thenReturn(1);

        service.save(USER_ID, today, blankRequest(List.of("calf")));

        verify(mapper).deleteById(USER_ID, 4);
        verify(mapper, never()).update(anyLong(), any());
    }
    ...
    void validatesDatePrecisionAndMeasurementType() {
        ...
        assertThrows(IllegalArgumentException.class,
                () -> service.getTrend(USER_ID, "unknown", today.minusDays(6), today, today));
    }
    ...
    void buildsFieldSpecificTrendWithBlankAndCarriedDates() {
        ...
        when(mapper.findFirstDateForColumn(USER_ID, "waist_cm")).thenReturn(first.getRecordDate());
        when(mapper.findLatestForColumn(USER_ID, start.minusDays(1), "waist_cm")).thenReturn(null);
        when(mapper.findBetweenForColumn(USER_ID, start, today, "waist_cm"))
                .thenReturn(List.of(first));

        BodyCircumferenceTrendDto trend =
                service.getTrend(USER_ID, "waist", start, today, today);
    ...
    void rejectsInvalidTrendRanges() {
        assertThrows(IllegalArgumentException.class,
                () -> service.getTrend(USER_ID, "waist", today, today.plusDays(1), today));
        assertThrows(IllegalArgumentException.class,
                () -> service.getTrend(USER_ID, "waist", today, today.minusDays(1), today));
        assertThrows(IllegalArgumentException.class,
                () -> service.getTrend(USER_ID, "waist", today.minusDays(365), today, today));
    }

    @Test
    void deleteCannotUseAnotherUsersRecord() {
